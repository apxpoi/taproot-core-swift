...
final class KeyDerivationTests: XCTestCase {
    ...
    func testIsDeterministicForSameInput() throws {
        ...
        let key1 = try KeyDerivation.deriveKey(password: password, salt: salt, iterations: 1000)
        let key2 = try KeyDerivation.deriveKey(password: password, salt: salt, iterations: 1000)
